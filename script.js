...
// Version: 3.2.1 (Comprehensive Fixes for Upload, List, Chat, and UI Layout)
...
const elements = {
    ...
    // Predefined questions container exists but is functionally unused/hidden
...
function resetAppState() {
    ...
    resetChatUI(); // This also handles deselecting chat items
}

function showDashboard() {
    elements.dashboardSection.style.display = 'flex';
    navigateToSection('uploadStructured'); // Default to structured data upload section on load
}
...
async function loadAllStructuredDocuments() {
    try {
        const data = await apiCall('/structured-documents'); // Corrected endpoint for listing
...
async function loadStructuredDocumentsForChat() {
    try {
        const structuredData = await apiCall('/structured-documents'); // Corrected endpoint for chat selection
        ...
        if (selectedChatStructuredDocumentId) {
            ...
            if (stillExists) {
                ...
                activateChatWithDocument(selectedChatStructuredDocumentId, selectedDocForChat?.filename, 'structured');
            } else {
                selectedChatStructuredDocumentId = null;
                resetChatUI();
            }
...
function activateChatWithDocument(docId, docFilename, docType) {
    ...
    document.querySelectorAll('#chat-structured-document-list .chat-document-item').forEach(item => {
        const isCurrentSelected = (item.dataset.docId === docId) && (item.dataset.docType === docType);
...
function initializeEventListeners() {
    ...
    Object.entries(elements.navLinks).forEach(([sectionName, navElement]) => {
        navElement?.addEventListener('click', (e) => {
            e.preventDefault();
            // sectionName directly maps to contentSections and navLinks IDs now
    ...
    elements.structuredDocumentsContainer?.addEventListener('click', (e) => {
        ...
        if (targetButton) {
            activateChatWithDocument(targetButton.dataset.docId, targetButton.dataset.docFilename, 'structured');
        }
    });
...
async function initializeApp() {
    ...
    try {
        ...
        showDashboard(); // Displays the first section ('uploadStructured') and sets up event listeners
    } catch (error) {
